...
describe("classifyAgainstBaseline", () => {
  ...
  it("classifies an only-baseline entry as fixed", () => {
    const e = entry({
      symbol: "deleted",
      fingerprint: "large_function::src/a.ts::deleted",
    });
    const result = classifyAgainstBaseline({
      baseline: [e],
      current: [],
...
describe("saveBaseline + loadBaseline (end-to-end against a tmp repo)", () => {
  ...
  it("writes a baseline to .crimes/baseline.json and reads it back", {
    timeout: 30000,
  }, async () => {
    await writeFile(join(repo, "biggie.ts"), bigFunctionSource("biggie"), "utf8");
    ...
    expect(result.baseline.findings.some((f) => f.symbol === "biggie")).toBe(true);
  ...
  it("loadBaseline throws BaselineNotFoundError when missing", async () => {
    ...
    await expect(loadBaseline(path)).rejects.toBeInstanceOf(BaselineNotFoundError);
  });
  ...
  it("loadBaseline throws MalformedBaselineError on invalid JSON", async () => {
    ...
    await expect(loadBaseline(path)).rejects.toBeInstanceOf(MalformedBaselineError);
  });
...
describe("checkBaseline (end-to-end against a tmp repo)", () => {
  ...
  it("reports new/fixed/unchanged correctly and does not fail when nothing new is added", {
    timeout: 30000,
  }, async () => {
  ...
  it("flags new findings and sets failed=true at the default medium threshold", {
    timeout: 30000,
  }, async () => {
    await writeFile(join(repo, "stable.ts"), bigFunctionSource("stableFn"), "utf8");
    ...
    expect(
      report.summary.new_by_severity.medium + report.summary.new_by_severity.high,
    ).toBeGreaterThan(0);
    expect(report.failed).toBe(true);
  });

  it("reports fixed findings when a file is deleted", { timeout: 30000 }, async () => {
    ...
    expect(report.fixed_findings.some((f) => f.symbol === "doomedFn")).toBe(true);
    expect(report.failed).toBe(false);
  });

  it("--fail-on high ignores new medium findings", { timeout: 30000 }, async () => {
    ...
    // medium.
    const todoBlock = Array.from({ length: 10 }, (_, i) => `// TODO item ${i}\n`).join(
      "",
    );
    await writeFile(join(repo, "todos.ts"), `${todoBlock}export const x = 1;\n`, "utf8");
