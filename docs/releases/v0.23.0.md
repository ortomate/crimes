...
Concretely, on hono's largest file the two hidden-failure findings (`swallowed_error` and `unsafe_retry`) now sit above three God
Function / God File findings that previously buried them.

## Eval baseline

| agent | 0.22.0 | 0.23.0 | move |
|---|---|---|---|
...
The accidental repeat sample at `0.22.0` measured this
...
The deterministic instrument is the one to read for this release: no
agent, no noise band, and the split by label above.
